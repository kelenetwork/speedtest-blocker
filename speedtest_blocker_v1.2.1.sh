...
# 是否已启用拦截（规则存在 + 服务正常 + 劫持生效）
is_block_enabled() {
    if [ -f "$DNSMASQ_CONF" ]; then
        grep -q "address=" "$DNSMASQ_CONF" || return 1
    else
        return 1
    fi

    check_iptables_status || return 1
    check_dnsmasq_status || return 1
...
show_status() {
    ...
    if is_block_enabled; then
        echo "[当前状态] ✅ 已启用测速网址拦截"
        echo "规则条数: $(grep -c '^address=' "$DNSMASQ_CONF") 条"
        echo "dnsmasq 状态: $(systemctl is-active dnsmasq)"
    else
        echo "[当前状态] ⛔ 未启用测速网址拦截"
    fi
    echo
